...
class AnimationController {
  ...
  private drawStartDot() {
    if (this.time > this.changeEventTime) {
      const dy =
        (this.cameraZ * this.startDotYOffset) / this.viewZoom;
...
class Star {
  ...
  constructor(cameraZ: number, cameraTravelDistance: number) {
    ...
    this.spiralLocation =
      (1 - Math.pow(1 - Math.random(), 3.0)) / 1.3;
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      if (displacementProgress < 0.3) {
        ...
        const targetX =
          spiralPos.x + targetDistance * Math.cos(spiralAngle);
        const targetY =
          spiralPos.y + targetDistance * Math.sin(spiralAngle);
...
export function SpiralAnimation({ containerRef, className }: SpiralAnimationProps) {
  ...
  useEffect(() => {
    ...
    const size = Math.max(dimensions.width, dimensions.height, 400);
